...
class MetricsExporter:
    ...
    def export_aov_to_horizontal_csv(self, aov_metrics: Dict[str, Any], 
                                    ...
                                    filename: str = None) -> str:
        """
        Export AOV metrics to a CSV file in a horizontal layout as requested.
        ...
        try:
            with open(filepath, 'w', newline='') as csvfile:
                ...
                # Write the column headers in a single row as requested
                headers = [
                    'Last 12 Months',
                ...
                # Write the values in a single row
                values = [
    ...
    def export_monthly_metrics_to_csv(self, monthly_metrics: Dict[str, Any], 
                                      ...
                                      filename: str = None) -> str:
        """
        Export monthly metrics to a CSV file in the specified format (14 rows with header, 12 months, and totals).
        ...
        try:
            with open(filepath, 'w', newline='') as csvfile:
                ...
                headers = [
                    ...
                    'New Customer Orders',
                    'New Customer AOV',
                    '%',  # Percentage change column for new customer AOV
                    'Returning Customer "Total Sales"',
                    'Returning Customer "Returns"',
                    'Total Returning Customer Revenue',
                    'Returning Customer Orders',
                    'Returning Customer AOV',
                    '%'   # Percentage change column for returning customer AOV
                ]
                ...
                # Write data for each month (rows 2-13)
                for month_key, month_data in sorted_months:
                    ...
                    if new_customer_percent is None:
                        new_customer_percent_str = ""  # Empty string for months with no orders
                    else:
                        new_customer_percent_str = f"{new_customer_percent * 100:.2f}%"
                        
                    if returning_customer_percent is None:
                        returning_customer_percent_str = ""  # Empty string for months with no orders
                    else:
                        returning_customer_percent_str = f"{returning_customer_percent * 100:.2f}%"
                    
                    # Build the row
                    row = [
                        month_info['display'],  # e.g., "January 2023"
                        ...
                        new_customer_percent_str,  # New customer AOV % change
                        ...
                        returning_customer_percent_str  # Returning customer AOV % change
                    ]
                    writer.writerow(row)
                
                # Write the totals row (row 14)
                totals_row = [
                    "Total",
                    ...
                    "",  # Empty % for totals (since totals represent the baseline)
    ...
    def export_detailed_orders_to_csv(self, orders: List[Dict], 
        ...
        Args:
            ...
            start_date: Start date of the reporting period
            end_date: End date of the reporting period
            customer_types: Dictionary mapping customer IDs to boolean (True for new)
                ...
                for order in orders:
                    try:
                        ...
                        if order.get('customer') and order['customer'].get('id'):
                            ...
                            # Determine customer type if we have that information
                        ...
                        if order.get('totalPriceSet') and order['totalPriceSet'].get('shopMoney'):
                            sales_value = float(order['totalPriceSet']['shopMoney'].get('amount', 0.0))
                            
                        refunded = 0.0
                        if order.get('totalRefundedSet') and order['totalRefundedSet'].get('shopMoney'):
                            refunded = float(order['totalRefundedSet']['shopMoney'].get('amount', 0.0))
                            
                        # Calculate total revenue (sales + refunds)
                        total_revenue = sales_value + refunded
                        
                        # Notes for context
                        notes = ""
                        if refunded > 0:
                            notes = "Includes refunded amount in revenue calculation"
